...
import { Sparkles, Type, Eye, Maximize2, Target } from 'lucide-react';
            ...
            <div className='flex-1 flex items-center justify-center'>
                <div className='text-center'>
                    <div className='w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4'>
                        <Type className='w-8 h-8 text-gray-400'></Type>
                    </div>
                        ...
                        <div className='flex items-center gap-3'>
                            <div className='flex items-center border border-gray-200 rounded-lg overflow-hidden'>
                                <button onClick={() => setIsPreviewMode(false)}
                                    className={`px-3 py-2 text-sm font-medium transition-colors ${isPreviewMode ? 'bg-violet-50 text-violet-700 border-r border-violet-200'
                                        : 'text-gray-600 hover:bg-gray-50'
                                        }`}>
                                    Edit
                                </button>
                                <button onClick={() => setIsPreviewMode(true)}
                                    className={`px-3 py-2 text-sm font-medium transition-colors ${isPreviewMode ? 'bg-violet-50 text-violet-700' :
                                        'text-gray-600 hover:bg-gray-50'
                                        }`}>Preview</button>
                            </div>
                            ...
                            <Button onClick={() => onGenerateChapterContent(selectedChapterIndex)}
                                ...
                                icon={Sparkles}
                                size='sm'>Generate with AI </Button>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                {isPreviewMode?(
                                    ...
                                    <div className='h-full '>
                                        <SimpleMDEditor 
                                            ...
                                            options={mdeOptions}></SimpleMDEditor>
                                    </div>
                                )}
                            </div>
