...
const Testimonials = () => {
  return (
    <div id='testimonials' className='relative py-24 lg:py-32 bg-gradient-to-br from-violet-50 via-purple-50 to-white overflow-hidden'>
        <div className='absolute top-2 right-10 w-64 h-64 bg-violet-200/30 rounded-full blur-3xl'></div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                {TESTIMONIALS.map((testimonial, index) => {
                    return (
                      <div
                        key={index}
                        className='group relative bg-white/80 backdrop-blur-sm rounded-3xl p-8 border border-gray-100 hover:border-violet-200 hover:shadow-2xl hover:shadow-violet-500/10 transition-all duration-300 hover:-translate-y-1'
                      >
                        ...
                        <div className='absolute inset-0 bg-gradient-to-br from-violet-50/0 to-purple-50/0 group-hover:from-violet-50/0 group-hover:to-purple-50/30 rounded-3xl transition-all duration-300 -z-10'></div>
                      </div>
                    );
                })}
            </div>
