...
const BookCardSkeleton=()=>(
  <div className='animate-pulse bg-white border border-slate-200 rounded-lg shadow-sm'>
    ...
    <div className='p-4'>
      ...
      <div className='bg-slate-200 rounded w-1/2'></div>
    </div>
  </div>
);
      ...
      <div className='container mx-auto p-6'>
        ...
        {isLoading?(
          ...
          <div className='flex flex-col items-center justify-center py-12 text-center border-2 border-dashed border-slate-200 rounded-xl'>
            ...
            <Button onClick={handleCreateBookClick} icon={Plus}>Create Your First eBook</Button>
          </div>
        ):(
          <div className='grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
            {books.map((book)=>(
              ...
              key={book._id}
              book={book}
              onDelete={()=>setBookToDelete(book._id)}></BookCard>
            ))}
          </div>
        )}
        <Confirmationmodel 
        ...
        onBookCreated={handleBookCreated}></CreateBookModal>
      </div>
    </DashboardLayout>
