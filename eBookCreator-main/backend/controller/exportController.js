            ...
            paragraphs.push(
              new Paragraph({
                ...
                spacing: {
                  before: inList
                    ? 100
                    : DOCX_STYLES.spacing.paragraphBefore,
                  after: inList
                    ? 100
                    : DOCX_STYLES.spacing.paragraphAfter,
                  line: 360,
                },
...
const exportAsDocument = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${book.title.replace(/[^a-zA-Z0-9]/g, "")}.docx"`
    );
    ...
    if (book.chapters && book.chapters.length > 0) {
      book.chapters.forEach((chapter, index) => {
        try {
          ...
          doc.moveDown(
            TYPOGRAPHY.spacing.chapterSpacing /
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      if (token.type === "heading_open") {
        ...
        doc.moveDown(
          TYPOGRAPHY.spacing.headingSpacing.before /
            TYPOGRAPHY.sizes.body
        );
        ...
        doc.moveDown(
          TYPOGRAPHY.spacing.headingSpacing.after /
            TYPOGRAPHY.sizes.body
        );
        ...
        if (!inList) {
          doc.moveDown(
            TYPOGRAPHY.spacing.paragraphSpacing /
              TYPOGRAPHY.sizes.body
          );
        }
        ...
        listType = "bullet";
        doc.moveDown(
          TYPOGRAPHY.spacing.listSpacing /
            TYPOGRAPHY.sizes.body
        );
      }
